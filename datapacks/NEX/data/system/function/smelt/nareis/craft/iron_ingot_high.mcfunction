...
execute store result score #mat1 shop run clear @s minecraft:coal_block[custom_data={nexoria_item:"high_coal"}] 0
execute unless score #mat1 shop matches 1.. run scoreboard players set #craft shop 0
execute if score #craft shop matches 1 run clear @s minecraft:iron_block[custom_data={nexoria_item:"high_iron"}] 1
execute if score #craft shop matches 1 run clear @s minecraft:coal_block[custom_data={nexoria_item:"high_coal"}] 1
