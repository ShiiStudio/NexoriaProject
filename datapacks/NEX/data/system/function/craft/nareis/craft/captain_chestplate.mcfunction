...
execute if score #craft shop matches 1 run clear @s minecraft:echo_shard[custom_data={nexoria_item:"captain_emblem"}] 4
execute if score #craft shop matches 1 run clear @s minecraft:iron_nugget[custom_data={nexoria_item:"old_iron"}] 8
execute if score #craft shop matches 1 run give @s minecraft:iron_chestplate[custom_name='{"text":"隊長の胸当て","color":"blue","italic":false}',custom_data={nexoria_armor:"captain_chestplate",hp:22},attribute_modifiers=[{id:"captain_chestplate_hp",type:"minecraft:generic.max_health",amount:22,operation:"add_value",slot:"chest"}],unbreakable={},trim={pattern:"minecraft:sentry",material:"minecraft:copper"}] 1
execute if score #craft shop matches 1 run tellraw @s {"text":"✔ 隊長の胸当てを作成しました。","color":"green"}
