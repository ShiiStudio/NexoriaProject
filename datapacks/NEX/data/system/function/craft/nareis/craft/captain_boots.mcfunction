...
execute store result score #mat2 shop run clear @s minecraft:light_blue_wool[custom_data={nexoria_item:"wet_cloth"}] 0
...
execute if score #craft shop matches 1 run give @s minecraft:iron_boots[custom_name='{"text":"隊長のブーツ","color":"blue","italic":false}',custom_data={nexoria_armor:"captain_boots",hp:10},attribute_modifiers=[{id:"captain_boots_hp",type:"minecraft:generic.max_health",amount:10,operation:"add_value",slot:"feet"}],unbreakable={},trim={pattern:"minecraft:sentry",material:"minecraft:copper"}] 1
execute if score #craft shop matches 1 run tellraw @s {"text":"✔ 隊長のブーツを作成しました。","color":"green"}
