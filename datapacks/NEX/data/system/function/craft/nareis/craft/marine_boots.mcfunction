...
execute if score #craft shop matches 1 run clear @s minecraft:prismarine_crystals[custom_data={nexoria_item:"sea_crystal"}] 6
execute if score #craft shop matches 1 run give @s minecraft:iron_boots[custom_name='{"text":"海兵のブーツ","color":"green","italic":false}',custom_data={nexoria_armor:"marine_boots",hp:8},attribute_modifiers=[{id:"marine_boots_hp",type:"minecraft:generic.max_health",amount:8,operation:"add_value",slot:"feet"}],unbreakable={},trim={pattern:"minecraft:coast",material:"minecraft:diamond"}] 1
execute if score #craft shop matches 1 run tellraw @s {"text":"✔ 海兵のブーツを作成しました。","color":"green"}
