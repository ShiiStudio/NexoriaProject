scoreboard players set #craft shop 1
execute store result score #mat0 shop run clear @s minecraft:iron_ingot[custom_data={nexoria_material:"iron_ingot"}] 0
execute unless score #mat0 shop matches 5.. run scoreboard players set #craft shop 0
execute store result score #mat1 shop run clear @s minecraft:string[custom_data={nexoria_item:"spider_string"}] 0
execute unless score #mat1 shop matches 10.. run scoreboard players set #craft shop 0
execute if score #craft shop matches 1 run clear @s minecraft:iron_ingot[custom_data={nexoria_material:"iron_ingot"}] 5
execute if score #craft shop matches 1 run clear @s minecraft:string[custom_data={nexoria_item:"spider_string"}] 10
execute if score #craft shop matches 1 run give @s minecraft:iron_helmet[custom_name='{"text":"探索者の兜","color":"green","italic":false}',custom_data={nexoria_armor:"explorer_helmet",hp:7},attribute_modifiers=[{id:"explorer_helmet_hp",type:"minecraft:generic.max_health",amount:7,operation:"add_value",slot:"head"}],unbreakable={},trim={pattern:"minecraft:sentry",material:"minecraft:copper"}] 1
execute if score #craft shop matches 1 run tellraw @s {"text":"✔ 探索者の兜を作成しました。","color":"green"}
execute unless score #craft shop matches 1 run tellraw @s {"text":"素材またはGoldが足りません。","color":"red"}
