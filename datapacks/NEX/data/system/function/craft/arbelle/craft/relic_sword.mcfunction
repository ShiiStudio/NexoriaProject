...
execute store result score #mat3 shop run clear @s minecraft:bone[custom_data={nexoria_item:"strong_bone"}] 0
...
execute if score #craft shop matches 1 run give @s minecraft:iron_sword[custom_name='{"text":"遺跡の剣","color":"blue","italic":false}',custom_data={nexoria_weapon:"relic_sword",atk:9},attribute_modifiers=[{id:"relic_sword_atk",type:"minecraft:generic.attack_damage",amount:9,operation:"add_value",slot:"mainhand"},{id:"relic_sword_speed",type:"minecraft:generic.attack_speed",amount:-2.4,operation:"add_value",slot:"mainhand"}],unbreakable={}] 1
execute if score #craft shop matches 1 run tellraw @s {"text":"✔ 遺跡の剣を作成しました。","color":"green"}
