...
execute store result score #mat1 shop run clear @s minecraft:iron_nugget[custom_data={nexoria_item:"rusted_iron"}] 0
...
execute if score #craft shop matches 1 run clear @s minecraft:iron_nugget[custom_data={nexoria_item:"rusted_iron"}] 10
execute if score #craft shop matches 1 run clear @s minecraft:bone[custom_data={nexoria_item:"strong_bone"}] 8
execute if score #craft shop matches 1 run give @s minecraft:iron_chestplate[custom_name='{"text":"騎士の胸当て","color":"blue","italic":false}',custom_data={nexoria_armor:"knight_chestplate",hp:16},attribute_modifiers=[{id:"knight_chestplate_hp",type:"minecraft:generic.max_health",amount:16,operation:"add_value",slot:"chest"}],unbreakable={}] 1
execute if score #craft shop matches 1 run tellraw @s {"text":"✔ 騎士の胸当てを作成しました。","color":"green"}
