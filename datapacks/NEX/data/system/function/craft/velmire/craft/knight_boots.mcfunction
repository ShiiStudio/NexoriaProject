...
execute store result score #mat1 shop run clear @s minecraft:iron_nugget[custom_data={nexoria_item:"rusted_knight_iron"}] 0
...
execute if score #craft shop matches 1 run clear @s minecraft:iron_nugget[custom_data={nexoria_item:"rusted_knight_iron"}] 4
execute if score #craft shop matches 1 run clear @s minecraft:bone[custom_data={nexoria_item:"strong_bone"}] 4
execute if score #craft shop matches 1 run give @s minecraft:chainmail_boots[custom_name='{"text":"騎士のブーツ","color":"blue","italic":false}',custom_data={nexoria_armor:"knight_boots",hp:7},attribute_modifiers=[{id:"knight_boots_hp",type:"minecraft:generic.max_health",amount:7,operation:"add_value",slot:"feet"}],unbreakable={}] 1
execute if score #craft shop matches 1 run tellraw @s {"text":"✔ 騎士のブーツを作成しました。","color":"green"}
