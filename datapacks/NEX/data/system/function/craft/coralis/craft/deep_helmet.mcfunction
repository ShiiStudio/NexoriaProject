...
execute if score #craft shop matches 1 run clear @s minecraft:prismarine_shard[custom_data={nexoria_item:"deep_spike"}] 4
execute if score #craft shop matches 1 run give @s minecraft:diamond_helmet[custom_name='{"text":"深海の兜","color":"green","italic":false}',custom_data={nexoria_armor:"deep_helmet",hp:10},attribute_modifiers=[{id:"deep_helmet_hp",type:"minecraft:generic.max_health",amount:10,operation:"add_value",slot:"head"}],unbreakable={},trim={pattern:"minecraft:coast",material:"minecraft:lapis"}] 1
execute if score #craft shop matches 1 run tellraw @s {"text":"✔ 深海の兜を作成しました。","color":"green"}
